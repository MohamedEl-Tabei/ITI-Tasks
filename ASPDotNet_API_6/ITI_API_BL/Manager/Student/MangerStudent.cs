...
public class MangerStudent : IManagerStudent
{
    ...
    public MangerStudent(IUnitOfWork repo, Validator validations)
    {
        Repo = repo;
        _validator = validations;
    }

    public GeneralResult<DTOStudentCreate> Add(DTOStudentCreate s)
    {
        var x = _validator.Validate(s);
        ...
        return new GeneralResult<DTOStudentCreate>()
        {
            Success = true
            ,Data=s
        };
    ...
    public void Update(Guid id, DTOStudentCreate newData)
    {
